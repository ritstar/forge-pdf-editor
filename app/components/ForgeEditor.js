...
function createImageElement(file, pageIndex, isSignature = false) {
  return {
    id: makeId(),
    type: 'image',
    pageIndex,
...
export default function Home() {
  ...
  const addImage = (file, isSignature = false) => {
    if (!file || !file.type.startsWith('image/')) return;
    const item = createImageElement(file, currentPage - 1, isSignature);
    applyChange((prev) => [...prev, item]);
    setSelectedId(item.id);
  };

  const handleImageInput = (event) => {
    addImage(event.target.files?.[0], false);
    event.target.value = '';
  };
  ...
  const handleSignatureInput = async (event) => {
    ...
    try {
      const processed = await removeWhiteBackground(rawFile);
      addImage(processed, true);
    } catch {
      addImage(rawFile, true);
    }
  };
  ...
  const exportImage = async (format) => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = dataUrl;
      link.download = `${pdfFile.name.replace(/\.pdf$/i, '')}-page-${currentPage}.${format === 'jpeg' ? 'jpg' : format}`;
      link.click();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="hero">
        <h1>Forge PDF Editor</h1>
        <p>Multi-page PDF editing with text, images, signatures, resize, keyboard controls, and clean exports.</p>
      </section>
      ...
      {!pdfFile ? (
        ...
        >
          ...
          <p>or choose a file from disk</p>
          <label className="primary-btn" htmlFor="pdf-upload">
            Select PDF
          </label>
          <input
            id="pdf-upload"
        ...
        <section className="workspace" onMouseDown={() => setSelectedId(null)}>
          <div className="topbar">
            ...
            <div className="topbar-actions">
              ...
              <button className="ghost-btn" onClick={resetEditor}>
                <RotateCcw size={16} /> New PDF
              </button>
            </div>
          </div>
          ...
          <div className="tool-row">
            ...
            <button className="action-btn" onClick={() => signatureInputRef.current?.click()}>
              <Signature size={16} /> Add Signature
            </button>
                          ...
                          {selectedId === item.id ? (
                            <>
                              <button
                                ...
                                onMouseDown={(event) => startInteraction(event, item.id, 'resize', 'nw')}
                              />
                              <button
                                className="resize-handle ne"
                                onMouseDown={(event) => startInteraction(event, item.id, 'resize', 'ne')}
                              />
                              <button
                                className="resize-handle sw"
                                onMouseDown={(event) => startInteraction(event, item.id, 'resize', 'sw')}
                              />
                              <button
                                className="resize-handle se"
                                onMouseDown={(event) => startInteraction(event, item.id, 'resize', 'se')}
                              />
                            </>
                          ) : null}
                        </div>
              ...
              <div className="panel-section">
                ...
                {!selectedElement ? (
                  ...
                  <>
                    ...
                    {selectedElement.type === 'text' ? (
                      <>
                        <label className="field-label" htmlFor="text-content-field">
                          Text
                        </label>
                        <textarea
                        ...
                        <label className="field-label" htmlFor="text-color-field">
                          Color
                        </label>
                        <input
                          id="text-color-field"
                        ...
                        <button
                          className="ghost-btn"
                          onClick={() => updateSelected({ bold: !selectedElement.bold })}
                        >
